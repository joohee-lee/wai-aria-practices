...
# 정리. 
- 디바이스가 다양해지고, 많은 양의 복잡한 데이터를 담기 위해 점점 복잡한 UI가 늘어나고 있는 환경 및 개발요청으로 버튼기능이나 링크 기능을 줘야 할 경우 등... 
  시멘틱한 마크업만으로는 해결할 수 없는 접근성 이슈들이 wai-aria 적용으로 접근성 이슈를 해결할 수 있다. 
- 항상 기본 마크업은 네이티브 요소를 최우선으로 최대한 사용하는 것이며, 
wai-aria 는 차선책으로 고민하는 것이 좋다고 생각한다.  
- 잘못된 적용사례는 사용하느니 못하다. 과유불급!!
